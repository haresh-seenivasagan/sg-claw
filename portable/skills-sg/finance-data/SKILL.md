---
name: finance-data
description: "Fetch professional stock market data from Yahoo Finance (yfinance) and SEC EDGAR. Use when: user asks about stock prices, market data, company financials, earnings press releases, quarterly results announcements, latest 10-Q / 10-K / 8-K filings, analyst recommendations, insider transactions, options chains, dividend history, company profiles, XBRL financial concepts, or any equity research task. Includes a one-call shortcut for earnings press releases (8k-exhibit subcommand). Supports US stocks, Chinese A-shares (e.g. 600519.SS), and international markets."
...
### Earnings Press Release (8-K Exhibit)

Fetch the press release attached to the latest earnings 8-K (filings with Item 2.02 = "Results of Operations"). This is the **fastest path to a company's quarterly results narrative** — short, in plain English, contains revenue / EPS / guidance.

```bash
# Latest earnings press release for NVDA
python3 scripts/sec_edgar.py 8k-exhibit NVDA

# Pin a specific quarter by announcement date prefix (YYYY-MM)
# e.g. AAPL Q3 FY25 was announced ~late July 2025
python3 scripts/sec_edgar.py 8k-exhibit AAPL --period 2025-07

# Different exhibit (default is 99.1 = press release)
python3 scripts/sec_edgar.py 8k-exhibit AVGO --exhibit 99.2
```

The `--period` filter matches the 8-K's `reportDate` (the announcement date), which is typically a few weeks **after** the fiscal quarter end:

| Company | FY ends | Q3 FY25 announced ~ |
| ------- | ------- | ------------------- |
| AAPL    | late Sep | 2025-07 (Q3 FY25 = quarter ending Jun) |
| NVDA    | late Jan | 2025-11 (Q3 FY26 = quarter ending Oct) |
| AVGO    | late Oct | 2025-09 (Q3 FY25 = quarter ending Aug) |
| Most    | calendar | Q1=Apr, Q2=Jul, Q3=Oct, Q4=Feb |

If `--period` returns no match, list 8-Ks manually with `filings <ticker> --form-type 8-K` and look for the one with `"items": "2.02..."` to find the right announcement date.

The output includes a `content` field with the press release text (default capped at 50,000 chars; raise with `--max-chars`).
